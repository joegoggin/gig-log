...
use super::payloads::{
    CompaniesListResponse, CompanyResponse, CreateCompanyRequest, DeleteCompanyResponse,
...
///
/// - `company` - The requested company resource
///
/// # Errors
///
/// - `Unauthorized` - If the request does not include a valid access token
...
mod tests {
    ...
    #[actix_web::test]
    // Verifies malformed UUID path segments are rejected before DB access.
    async fn get_company_with_invalid_uuid_path_returns_bad_request() {
        let state = test_state();
        let access_token = create_access_token(
            Uuid::new_v4(),
            "company-handler@test.dev",
