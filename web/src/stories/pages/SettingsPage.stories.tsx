 ...
 * - Theme and palette controls update `data-theme`/`data-palette`.
 * - Contrast checks verify readable text/surface pairs across palettes.
 *
 * Regression focus:
...
export const MaintainsReadableContrastAcrossPalettesAndThemes: Story = {
    ...
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        const rootElement = canvasElement.ownerDocument.documentElement;
        const heading = canvas.getByRole("heading", { level: 1, name: "Settings" });
        const primaryActionButton = canvas.getByRole("button", {
        ...
        for (const themeSelection of themeSelections) {
            ...
            for (const paletteSelection of paletteSelections) {
                ...
                const buttonContrast = getContrastRatio(
                    getComputedStyle(primaryActionButton).color,
                    getComputedStyle(primaryActionButton).backgroundColor,
                );

                await expect(pageContrast).toBeGreaterThanOrEqual(4.5);
                await expect(buttonContrast).toBeGreaterThanOrEqual(4.5);
            }
        }
    },
};
